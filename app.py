...
def handle_data_cleaning():
    ...
    if st.button("💾 Save Version Snapshot", disabled=st.session_state.cleaned_data is None):
        if st.session_state.cleaned_data is None:
            st.warning("Clean data before saving a version snapshot.")
        else:
...
def handle_storage_and_queries():
    ...
    if st.button("💾 Store Data in Database"):
        with st.spinner("Storing data..."):
            try:
                table_name = st.session_state.db_manager.store_data(st.session_state.cleaned_data, "data_table")
    ...
    if user_question and st.button("🔍 Analyze"):
        ...
        with st.spinner("Converting question to SQL and analyzing..."):
            try:
                st.session_state.db_manager.store_data(st.session_state.cleaned_data, "data_table")
